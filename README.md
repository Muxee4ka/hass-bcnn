...
После добавления ЛС интеграция создаёт устройство «ЛС №…» (с ЕЛС в качестве serial number, если он есть в кабинете) со следующими сущностями:

- **Сенсоры платежей:** «Лицевой счёт», «Сумма к оплате», «Задолженность», «Входящее сальдо», «Начислено», «Оплачено», «Дата начисления», «Последнее обновление». В атрибутах «Сумма к оплате» — детализация по услугам.
- **Сенсоры счётчиков:** на каждый прибор учёта создаются два сенсора — текущие показания (м³) и «… — потребление» (`state_class=total_increasing`, подключается к HA Water Dashboard).
- **Бинарный сенсор «Есть задолженность»** (`device_class=problem`): включён, когда «К оплате» больше нуля.
